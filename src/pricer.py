    ...
    for i, item in enumerate(project_boq):
        ...
        if match:
            ...
            if use_ai_fallback:
                ...
                project_boq[i]["rate_source"] = "manual-required"
                project_boq[i]["notes"] = (
                    (project_boq[i].get("notes") or "") +
...
def _find_rate_match(
    ...
    item_unit = str(item.get("unit", "")).lower().strip()

    # Normalise plurals: "clips"→"clip", "screws"→"screw", etc.
    def _stem(words):
        return {w[:-1] if w.endswith("s") and len(w) > 4 else w for w in words}

    item_words_raw = set(re.findall(r'\b[a-z]{3,}\b', item_desc))
    item_words = _stem(item_words_raw)
    ...
    for entry in rate_library:
        ...
        entry_unit = str(entry.get("unit", "")).lower().strip()
        entry_words_raw = set(re.findall(r'\b[a-z]{3,}\b', entry_desc))
        entry_words = _stem(entry_words_raw)
        ...
        # Bonus when units match (e.g. "each" vs "each") — prevents
        # matching length items to count items and vice versa
        if item_unit and entry_unit and item_unit == entry_unit:
            score += 0.25
